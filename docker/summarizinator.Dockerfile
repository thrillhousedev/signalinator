# Summarizinator - AI-powered message summarization (requires Ollama)
...
COPY bots/summarizinator/ bots/summarizinator/

RUN pip install --no-cache-dir "packages/signalinator-core[encryption]" \
    && pip install --no-cache-dir bots/summarizinator

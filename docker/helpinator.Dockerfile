...
COPY bots/helpinator/ bots/helpinator/

RUN pip install --no-cache-dir "packages/signalinator-core[encryption]" \
    && pip install --no-cache-dir bots/helpinator
...
COPY --from=builder /usr/local/bin/helpinator /usr/local/bin/

ENV PYTHONUNBUFFERED=1
ENV LOG_LEVEL=INFO

CMD ["helpinator", "daemon"]

...
| [**helpinator**](bots/helpinator/) | Signal help desk. User DMs open tickets in a control room with ticket numbers, subjects, immutable internal notes, and agent-initiated close with a resolution DM to the user. CSV and Markdown export. Can also run as plain message relay. |
...
| [**snackinator**](bots/snackinator/) | AI-powered snack and meal recommendations via Ollama. Works in groups (@mention) and DMs. Non-judgmental, concise, and dry-witted. |
...
| [**transcribinator**](bots/transcribinator/) | Audio/video transcription using local Whisper. Optional AI cleanup and summarization via Ollama. Supports voice notes, audio files, and video. |
...
- **For AI bots** (summarizinator, informationator, snackinator, transcribinator): [Ollama](https://ollama.com) running locally
...
6. Set the bot's profile:
   ...
   **Option A: Via daemon commands** (recommended for initial setup):
   ...
   # Set profile photo (copy image into config volume first)
   cp avatar.png ./config/taginator/avatar.png
   docker compose run --rm taginator-daemon profile --name "Taginator" --avatar /signal-cli-config/avatar.png
...
> **Note**: These instructions work for any bot. Replace `taginator` with the bot you're setting up (e.g., `newsinator`, `summarizinator`, `helpinator`). Each bot must be registered separately with its own phone number.
...
The summarizinator, informationator, snackinator, and transcribinator bots require an Ollama server:

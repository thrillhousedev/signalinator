...
class TestTaginatorBotLifecycle:
    ...
    def test_handle_group_message_with_text_triggers_tag(self, bot):
        """Test that @mention with surrounding text still triggers tagging."""
        bot._bot_uuid = "bot-uuid"
        bot.list_groups = MagicMock(return_value=[
            {"id": "group-123", "name": "Test Group", "members": [
                {"uuid": "user-1"},
                {"uuid": "user-2"},
            ], "admins": [{"uuid": "user-1"}]},
        ])
        ...
        # Should trigger tagging (returns None when mentions sent directly)
        assert result is None
        bot.send_message.assert_called()

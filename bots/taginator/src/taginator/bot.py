...
from signalinator_core.bot.command_router import get_group_members, check_group_admin
...
class TaginatorBot(SignalinatorBot):
    ...
    def _get_help_text(self) -> str:
        ...
        return (
            "Taginator - @Mention Everyone\n\n"
            "📢 Tagging\n"
            "@mention - @mention all group members\n"
            "/tag - Same as @mention\n\n"
            "⚙️ Control (Admin)\n"
            "/pause - Pause tagging\n"
            "/unpause - Resume tagging\n\n"
            "🤖 Profile Settings\n"
    ...
    def handle_group_message(
        ...
        Bare @mention (no text) triggers tagging everyone.
        @mention with text shows help.
        """
        # Check if message has content beyond the mention placeholder
        # Signal uses \uFFFC as the mention placeholder character
        message_text = (context.message or "").replace("\uFFFC", "").strip()
        if message_text:
            return self._get_help_text()

        # Bare @mention - trigger tag functionality
        if not context.group_id:
            return "Tagging only works in groups."

        # Fetch groups and check admin status
    ...
    def on_group_joined(self, group_id: str, group_name: str) -> Optional[str]:
        ...
        return "👋 Hi! I'm Taginator. @mention me to ping everyone, or /help for commands."
    ...
    def _perform_tag(
        ...
        """Core tag logic - @mention all group members.
        ...
        Returns:
            Error message string, or None if mentions were sent successfully
        """
        # Check if paused
        ...
        # Get group members
        members = get_group_members(group_id, groups)

"""Session lifecycle management for Helpinator."""
...
class SessionManager:
    ...
    def __init__(self, db: HelpinatorRepository):
        self.db = db
    ...
    def handle_member_join(
        ...
        if existing:
            ...
            if user_name and user_name != existing.user_name:
    ...
    def get_display_name(
        ...
        if session.is_direct_dm or room_pair is None:
            # Per-session override takes precedence over room-level setting
            effective_anonymous = dm_anonymous
            if session.anonymous_override is not None:
                effective_anonymous = session.anonymous_override
            if effective_anonymous and session.pseudonym:
                return session.pseudonym
            if session.user_name:
                return session.user_name
            # Never display phone number for privacy - fall back to UUID

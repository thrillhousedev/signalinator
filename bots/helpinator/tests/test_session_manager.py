"""Tests for Helpinator session manager."""
...
class TestSessionManager:
    ...
    def test_get_display_name_fallback_no_name(self, sample_session, sample_room_pair):
        """Falls back to truncated UUID (not phone number) for privacy."""
        sample_room_pair.anonymous_mode = False
        ...
        # Privacy: never display phone number, fall back to UUID
        assert result == sample_session.user_uuid[:8] + "..."

    def test_get_display_name_dm_anonymous_override_false(self, sample_session):
        """anonymous_override=False shows real name even when dm_anonymous is on."""
        sample_session.is_direct_dm = True
        sample_session.pseudonym = "DM-A"
        sample_session.anonymous_override = False
        sample_session.user_name = "Alice"

        result = self.manager.get_display_name(sample_session, room_pair=None, dm_anonymous=True)

        assert result == "Alice"

    def test_get_display_name_dm_anonymous_override_none(self, sample_session):
        """anonymous_override=None follows room-level dm_anonymous setting."""
        sample_session.is_direct_dm = True
        sample_session.pseudonym = "DM-A"
        sample_session.anonymous_override = None
        sample_session.user_name = "Alice"

        # Room-level anonymous on → pseudonym
        result = self.manager.get_display_name(sample_session, room_pair=None, dm_anonymous=True)
        assert result == "DM-A"

        # Room-level anonymous off → real name
        result = self.manager.get_display_name(sample_session, room_pair=None, dm_anonymous=False)
        assert result == "Alice"

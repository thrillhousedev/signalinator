"""Tests for Helpinator database repository."""
...
from helpinator.database.models import RoomPair, ActiveSession, RelayMapping


class TestHelpinatorRepositoryInit:
    """Tests for HelpinatorRepository initialization."""

    def test_creates_tables(self, tmp_path):
        """Creates all required tables on init."""
        db_path = str(tmp_path / "test.db")
        repo = HelpinatorRepository(create_engine(f"sqlite:///{db_path}"))
...
class TestRoomPairOperations:
    ...
    def repo(self):
        ...
        return HelpinatorRepository(engine)
...
class TestSessionOperations:
    ...
    def repo(self):
        engine = create_engine("sqlite:///:memory:")
        repo = HelpinatorRepository(engine)
        repo.create_room_pair("lobby-123", "control-456", "admin-uuid")
        return repo
    ...
    def test_dm_pseudonym_recycles_after_end(self, repo):
        """Ended DM sessions release pseudonyms back to the pool."""
        # Fill all 26 single-letter slots except DM-A
        for i, c in enumerate(string.ascii_uppercase):
            if c != "A":
                repo.create_direct_dm_session(f"user-fill-{i}", pseudonym=f"DM-{c}")

        # Create and end DM-A session
        session = repo.create_direct_dm_session("user-1", "Alice", "+15551234567", pseudonym="DM-A")
        repo.end_session(session.id)

        # Only DM-A is available (all others are active), so it must be returned
        pseudonym = repo.get_next_dm_pseudonym()
        assert pseudonym == "DM-A"

    def test_dm_pseudonym_not_reused_while_active(self, repo):
        """Active DM sessions keep their pseudonym out of the pool."""
        repo.create_direct_dm_session("user-1", "Alice", "+15551234567", pseudonym="DM-A")

        # DM-A should not be available since session is active
        pseudonyms = set()
        for _ in range(50):
            pseudonyms.add(repo.get_next_dm_pseudonym())
        assert "DM-A" not in pseudonyms
...
class TestRelayMappingOperations:
    ...
    def test_delete_session_relay_mappings(self, repo, session):
        """Deletes all relay mappings for a session."""
        repo.create_relay_mapping(session.id, 1700000001000, "user-uuid-1", "to_control")
        repo.create_relay_mapping(session.id, 1700000002000, "user-uuid-1", "to_control")

        deleted = repo.delete_session_relay_mappings(session.id)
        assert deleted == 2

        # Mappings should be gone
        assert repo.get_relay_mapping_by_timestamp(1700000001000) is None
        assert repo.get_relay_mapping_by_timestamp(1700000002000) is None

    def test_delete_session_relay_mappings_none(self, repo, session):
        """Returns 0 when no mappings exist."""
        deleted = repo.delete_session_relay_mappings(session.id)
        assert deleted == 0


class TestSessionListing:
    """Tests for session listing and purge."""

    @pytest.fixture
    def repo(self):
        engine = create_engine("sqlite:///:memory:")
        repo = HelpinatorRepository(engine)
        repo.create_room_pair("lobby-123", "control-456", "admin")
        return repo
    ...
    def test_get_all_active_sessions(self, repo, pair):
        """Returns all active sessions ordered by last_activity DESC."""
        s1 = repo.create_session(pair.id, "user-1", "Alice")
        s2 = repo.create_session(pair.id, "user-2", "Bob")
        # Update s1 to be more recent
        repo.update_session(s1.id, last_activity=datetime.now(timezone.utc))

        sessions = repo.get_all_active_sessions()
        assert len(sessions) == 2
        # s1 updated more recently, should be first
        assert sessions[0].user_name == "Alice"

    def test_get_all_active_sessions_excludes_left(self, repo, pair):
    ...
    def test_purge_stale_sessions(self, repo, pair):
        """Purges sessions inactive for longer than retention period."""
        s1 = repo.create_session(pair.id, "user-1", "Alice")
        repo.create_session(pair.id, "user-2", "Bob")
        # Make s1 stale (8 days old)
        old_time = datetime.now(timezone.utc) - timedelta(days=8)
        repo.update_session(s1.id, last_activity=old_time)

        purged = repo.purge_stale_sessions(days=7)
        assert purged == 1
    ...
    def test_purge_stale_sessions_deletes_mappings(self, repo, pair):
        """Purging also deletes relay mappings for stale sessions."""
        s1 = repo.create_session(pair.id, "user-1", "Alice")
        repo.create_relay_mapping(s1.id, 1700000001000, "user-1", "to_control")
        old_time = datetime.now(timezone.utc) - timedelta(days=8)
        repo.update_session(s1.id, last_activity=old_time)

        repo.purge_stale_sessions(days=7)
        assert repo.get_relay_mapping_by_timestamp(1700000001000) is None

    def test_purge_stale_sessions_none_stale(self, repo, pair):
        """Returns 0 when no sessions are stale."""
        repo.create_session(pair.id, "user-1", "Alice")
        purged = repo.purge_stale_sessions(days=7)
        assert purged == 0

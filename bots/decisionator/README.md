...
| Command | Description |
|---------|-------------|
| `?register [name]` | Link your Loomio account (optional display name) |
| `?status` | Check registration status |
| `?help` | Show all commands |

The first user to `?register` from a Signal group automatically creates and links a matching Loomio group; subsequent registrations from the same Signal group join that existing Loomio group. The bot reports the linking outcome in the `?register` response — including any Loomio API error so operators can diagnose failures.

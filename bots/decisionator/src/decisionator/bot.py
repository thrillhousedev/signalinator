...
class DecisionatorBot(SignalinatorBot):
    ...
    def get_commands(self) -> Dict[str, BotCommand]:
        return {
            ...
            "/register": BotCommand(
                name="/register",
                description="Register with Loomio",
                handler=self._handle_register,
                usage="/register [loomio_username]",
            ),
            "/status": BotCommand(
                name="/status",
    ...
    def _handle_register(self, context: CommandContext) -> str:
        ...
        if existing:
            return f"✅ Already registered as Loomio user (ID: {existing.loomio_user_id})"

        if not args:
            return "Usage: /register <loomio_username>\nProvide your Loomio username to link accounts."

        try:
            user = self.loomio.lookup_user(args)
            if not user:
                return f"❌ Loomio user '{args}' not found. Check your username."

            self.repo.create_user_mapping(sender, user.id, user.username)
            return f"✅ Registered! Linked to Loomio user: {user.name} ({user.username})"
    ...
    def _handle_flow(self, context: CommandContext) -> str:
        ...
        try:
            ...
            threshold = group_mapping.consensus_threshold if group_mapping else 75
    ...
    def _handle_remind(self, context: CommandContext) -> str:
        ...
        if self.scheduler.send_poll_reminder(poll_id, context.message.group_id):
            return ""  # Reminder sent as separate message

...
class DecisionatorBot(SignalinatorBot):
    ...
    def _handle_register(self, context: CommandContext) -> str:
        ...
        if existing:
            # Ensure group is linked even if user already registered
            group_just_linked = False
            group_link_error: Optional[str] = None
            if context.message.group_id:
                loomio_group_id = self._get_loomio_group_id(context.message.group_id)
                if not loomio_group_id:
                    try:
                        group_name = context.message.group_name or "Signal Group"
                        loomio_group = self.loomio.create_group(name=group_name)
                        self.repo.create_group_mapping(
                            context.message.group_id,
                            loomio_group.id,
                            group_name=group_name,
                        )
                        loomio_group_id = loomio_group.id
                        group_just_linked = True
                        logger.info(f"Auto-created Loomio group '{group_name}' for Signal group")
                    except LoomioClientError as e:
                        group_link_error = str(e)
                        logger.warning(f"Failed to auto-create Loomio group: {e}")
                if loomio_group_id:
                    try:
                        self.loomio.add_member_to_group(loomio_group_id, existing.loomio_user_id)
            ...
            if custom_name:
                ...
                lines = [f"✅ You're already registered as @{username}"]
                if group_just_linked:
                    lines.append("🔗 This group is now linked to Loomio — go ahead and /propose.")
                elif group_link_error:
                    lines.append(f"⚠️ Couldn't link this group to Loomio: {group_link_error}")
                lines.append("")
                lines.append("💡 To change your display name:")
                lines.append("   /register Your New Name")
                return "\n".join(lines)
        ...
        try:
            ...
            if context.message.group_id:
                loomio_group_id = self._get_loomio_group_id(context.message.group_id)
                if not loomio_group_id:
                    # Auto-create Loomio group for this Signal group
                    group_name = context.message.group_name or "Signal Group"
                    loomio_group = self.loomio.create_group(name=group_name)
                    self.repo.create_group_mapping(
                        context.message.group_id,

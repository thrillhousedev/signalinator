...
class LoomioClient:
    ...
    def _request(
        ...
        """Make an API request."""
        url = f"{self.base_url}/api/b1/{endpoint.lstrip('/')}"
    ...
    def is_available(self) -> bool:
        """Check if Loomio API is accessible."""
        try:
            self._get("memberships")
            return True
    ...
    def lookup_user(self, username: str) -> Optional[User]:
        ...
        Note: Requires forked Loomio with /api/b1/users/{username} endpoint.
        """
        try:
    ...
    def update_user_name(self, username: str, new_name: str) -> bool:
        ...
        Note: Requires forked Loomio with /api/b1/users/{username} PATCH endpoint.
        """
        try:
            self._patch(f"users/{username}", {"name": new_name})
            return True
        except LoomioClientError:
            return False

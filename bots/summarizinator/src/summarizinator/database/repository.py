...
class SummarizinatorRepository:
    ...
    def _migrate_additive_columns(self) -> None:
        """Bring the live SQLite schema in line with the SQLAlchemy models.

        SQLAlchemy's `create_all()` creates missing tables but never alters
        existing ones, so additive column changes are invisible to bots that
        upgrade in place. For each table we own, query the live columns and
        ALTER ADD any that the model declares but the database lacks.

        Idempotent; safe to run on every startup. SQLite-only — other
        backends are no-op (the bots ship on SQLite/SQLCipher).
        """
        ...
        for table in Base.metadata.tables.values():
            ...
            for col in table.columns:
                if col.name in actual_cols:
                    continue
                ddl = self._column_add_ddl(col, dialect)
    ...
    def _column_add_ddl(col, dialect) -> str:
        """Render the column body of an ALTER TABLE ADD COLUMN statement.

        Translates the SQLAlchemy Column into SQLite DDL: name, type, optional
        NOT NULL, and a literal DEFAULT when the model declares a scalar
        Python default (booleans become 0/1, strings get single-quoted).
        """
        parts = [col.name, col.type.compile(dialect)]
        if not col.nullable:
    ...
    def set_capture_peer_bots(self, group_id: str, capture: bool) -> GroupSettings:
        """Set whether to capture messages from peer bots for this group.

        Creates the GroupSettings row if missing so the choice persists even
        before any other settings have been touched.
        """
        with self.get_session() as session:
            settings = session.query(GroupSettings).filter_by(group_id=group_id).first()
            if settings:
                settings.capture_peer_bots = capture
                settings.updated_at = datetime.now(timezone.utc)
            else:
                settings = GroupSettings(group_id=group_id, capture_peer_bots=capture)
                session.add(settings)
            session.flush()
            session.expunge(settings)
            return settings

    def set_power_mode(self, group_id: str, mode: str) -> GroupSettings:
        """Set who can run config commands ('admins' or 'everyone').

        Creates the GroupSettings row if missing so the choice persists even
        before the group has been touched by other commands.
        """
        if mode not in ("admins", "everyone"):
            raise ValueError(f"Invalid power_mode: {mode!r}. Use 'admins' or 'everyone'.")
        with self.get_session() as session:
            settings = session.query(GroupSettings).filter_by(group_id=group_id).first()
            if settings:
                ...
                settings = GroupSettings(group_id=group_id, power_mode=mode)
                session.add(settings)
            session.flush()
            session.expunge(settings)
            return settings
    ...
    def purge_expired_messages(self, default_retention_hours: int = 48) -> int:
        ...
        Compares against Signal's `signal_timestamp` (ms since epoch), not the
        local `received_at`. signal-cli replays buffered messages on reconnect
        with fresh receive times; using signal_timestamp ensures retention is
        measured from when the message was actually sent.
        """
        with self.get_session() as session:
            ...
            for group_id, retention_hours in retention_map.items():
                ...
                deleted = session.query(Message).filter(
                    ...
                    Message.signal_timestamp < cutoff_ms,
                ).delete()
                total_deleted += deleted

            default_cutoff_ms = now_ms - default_retention_hours * 3600 * 1000
            deleted = session.query(Message).filter(
                ~Message.group_id.in_(retention_map.keys()),
                Message.signal_timestamp < default_cutoff_ms,
            ).delete(synchronize_session=False)
            total_deleted += deleted

            return total_deleted

    def find_message_for_reaction(
        self,
        signal_timestamp: int,
        target_author_uuid: str,
        group_id: str,
    ) -> Optional[Message]:
        """Find the stored message that a reaction targets.

        Reactions reference the original message by (target_author, target_sent_timestamp).
        Combined with group_id this matches our message uniqueness constraint.
        """
        with self.get_session() as session:
            msg = session.query(Message).filter_by(
                signal_timestamp=signal_timestamp,
                sender_uuid=target_author_uuid,
                group_id=group_id,
            ).first()
            if msg:
                session.expunge(msg)
    ...
    def store_reaction(
        ...
        """Store a reaction, replacing any prior reaction from the same reactor.

        Signal sends a new reaction event when a user switches their emoji on a
        message. Keep only the latest reaction per (message, reactor).
        """
        with self.get_session() as session:
            session.query(Reaction).filter_by(
    ...
    def remove_reaction(self, message_id: int, reactor_uuid: str) -> int:
        """Remove all reactions from a given reactor on a message."""
        with self.get_session() as session:
            return session.query(Reaction).filter_by(
                message_id=message_id,
                reactor_uuid=reactor_uuid,
            ).delete()
    ...
    def get_messages_with_reactions_for_group(
        ...
        """Get messages with their reaction counts and emojis for a group.

        Returns a list of dicts shaped for the summarizer:
        {"id", "signal_timestamp", "sender_uuid", "content", "reaction_count", "emojis"}
        """
        with self.get_session() as session:
            query = session.query(Message).filter_by(group_id=group_id)
            ...
            query = query.order_by(Message.received_at)
            if limit:
            ...
            reactions_by_message: Dict[int, List[str]] = {}
            for r in reactions:
                reactions_by_message.setdefault(r.message_id, []).append(r.emoji)

            result = []
            for m in messages:
                emojis = reactions_by_message.get(m.id, [])
                result.append({
                    "id": m.id,
                    "signal_timestamp": m.signal_timestamp,
    ...
    def get_schedule_by_name(
        ...
        """Get a schedule by name for a source group."""
        with self.get_session() as session:
            schedule = session.query(ScheduledSummary).filter_by(
                name=name,
                source_group_id=source_group_id,
            ).first()
            if schedule:
    ...
    def find_group_by_name(self, name: str) -> Optional[GroupSettings]:
        """Find a group whose stored name matches (case-insensitive)."""
        with self.get_session() as session:
            group = session.query(GroupSettings).filter(
                GroupSettings.group_name.ilike(name)
            ).first()
            if group:
    ...
    def get_dm_message_count(self, user_id: str) -> int:
        ...
        with self.get_session() as session:
            return session.query(DMConversation).filter_by(
    ...
    def purge_expired_dm_messages(self, default_retention_hours: int = 48) -> int:
        ...
        Prefers Signal's `signal_timestamp` (ms since epoch) for the same
        replay-on-reconnect reason as group messages. Falls back to
        `created_at` for assistant-role rows where signal_timestamp is null.
        """
        with self.get_session() as session:
            ...
            def _expired(retention_hours: int):
                cutoff_ms = now_ms - retention_hours * 3600 * 1000
                cutoff_dt = now - timedelta(hours=retention_hours)
                return or_(
                    and_(
                        DMConversation.signal_timestamp.isnot(None),
                        DMConversation.signal_timestamp < cutoff_ms,
                    ),
                    and_(
                        DMConversation.signal_timestamp.is_(None),
                        DMConversation.created_at < cutoff_dt,
                    ),
                )
            ...
            for user_id, retention_hours in retention_map.items():
                deleted = session.query(DMConversation).filter(
                    ...
                    _expired(retention_hours),
                ).delete(synchronize_session=False)
                total_deleted += deleted

            deleted = session.query(DMConversation).filter(
                ~DMConversation.user_id.in_(retention_map.keys()),
                _expired(default_retention_hours),
            ).delete(synchronize_session=False)
            total_deleted += deleted

...
def _annotate_with_reactions(message: dict) -> str:
    """Append a [N reactions: emojis] suffix when a message has reactions."""
    content = message.get("content") or ""
    emojis = message.get("emojis") or []
    if not emojis:
        return content
    return f"{content} [{len(emojis)} reactions: {''.join(emojis)}]"
...
class SummaryScheduler:
    ...
    def _run_scheduled_summary(self, schedule_id: int):
        ...
        try:
            ...
            # Update last run time. (Purge-on-summary is handled inside
            # generate_and_post_summary so on-demand /summary honors it too.)
    ...
    def generate_and_post_summary(
        ...
        try:
            # Extract message texts annotated with reaction counts. The reactor
            # UUIDs stay in the DB only; only emoji counts surface to the LLM.
            messages_with_reactions = self.repo.get_messages_with_reactions_for_group(
                source_group_id, hours=hours,
            )
            message_texts = [
                _annotate_with_reactions(m) for m in messages_with_reactions
            ]
            ...
            # Send to target group (split to Signal's per-message limit if needed)
            for part in split_long_message(output):
                self.send_message(part, target_group_id)

            # Mark run complete
            self.repo.complete_summary_run(run.id, status="completed")

            # Purge source-group messages if /purge-mode is on. Lives here (not
            # only in _run_scheduled_summary) so on-demand /summary also honors
            # the setting.
            settings = self.repo.get_group_settings(source_group_id)
            if settings and settings.purge_on_summary:
                purged = self.repo.purge_messages(source_group_id)
                logger.info(
                    f"Purge-on-summary cleared {purged} message(s) from {source_group_id[:12]}..."
                )
    ...
    def _format_summary_message(
        ...
        lines.append("")
        lines.append("⚠️ Generated by offline AI — verify for accuracy.")

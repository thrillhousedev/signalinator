...
from summarizinator.ai.summarizer import ChatSummarizer, _validate_privacy
from summarizinator.ai.ollama_client import OllamaClient, OllamaClientError


class TestValidatePrivacy:
    """Tests for the post-generation privacy validator."""

    def test_empty_text_unchanged(self):
        assert _validate_privacy("", context="summary") == ""

    def test_clean_text_no_warning(self, caplog):
        with caplog.at_level(logging.WARNING):
            out = _validate_privacy("Participants discussed an upcoming deadline.", context="summary")
        assert out == "Participants discussed an upcoming deadline."
        assert not any("Privacy validation" in r.message for r in caplog.records)

    def test_leak_token_said_warns(self, caplog):
        with caplog.at_level(logging.WARNING):
            _validate_privacy("Someone said the deadline is Friday.", context="summary")
        assert any("'said'" in r.message for r in caplog.records)

    def test_at_mention_warns(self, caplog):
        with caplog.at_level(logging.WARNING):
            _validate_privacy("Mentioned by @alice in passing.", context="summary")
        # Both '@' and 'mentioned' should fire
        msgs = [r.message for r in caplog.records]
        assert any("'@'" in m for m in msgs)
        assert any("'mentioned'" in m for m in msgs)

    def test_quoted_span_warns(self, caplog):
        with caplog.at_level(logging.WARNING):
            _validate_privacy('A participant wrote "we should ship Friday".', context="summary")
        assert any("quoted span" in r.message for r in caplog.records)

    def test_short_quotes_not_flagged(self, caplog):
        """Tiny quoted spans (<4 chars inside quotes) should not trip the regex."""
        with caplog.at_level(logging.WARNING):
            _validate_privacy('Participants used "v1".', context="summary")
        assert not any("quoted span" in r.message for r in caplog.records)

    def test_returns_text_unchanged_even_on_warning(self):
        """Validator is non-destructive."""
        text = "Someone said hello."
        assert _validate_privacy(text, context="summary") == text
...
class TestSummarizeMessages:
    ...
    def test_summarize_messages_wraps_in_conversation_tags(self, summarizer, mock_ollama, sample_messages):
        """User content is wrapped in <conversation> tags for prompt-injection defense."""
        mock_ollama.generate.return_value = "ok"

        summarizer.summarize_messages(sample_messages)

        # `summarize_messages` issues several generate calls (summary, topics,
        # sentiment); find the one whose prompt actually summarizes the chat.
        summary_prompts = [
            call.kwargs.get("prompt", "")
            for call in mock_ollama.generate.call_args_list
            if "Summarize this group chat conversation" in call.kwargs.get("prompt", "")
        ]
        assert summary_prompts, "Expected a summary-prompt call to Ollama"
        prompt = summary_prompts[0]

        assert "<conversation>" in prompt
        assert "</conversation>" in prompt
        start = prompt.index("<conversation>")
        end = prompt.index("</conversation>")
        assert "Python 3.12" in prompt[start:end]

    def test_dm_system_prompt_defined(self):
        """ChatSummarizer exposes the DM system prompt for free-form DM chat."""
        assert hasattr(ChatSummarizer, "DM_SYSTEM_PROMPT")
        prompt = ChatSummarizer.DM_SYSTEM_PROMPT
        assert "Signal messenger" in prompt
...
- Real action item here"""
    ...
    def test_extract_action_items_ignores_prose(self, summarizer):
        """Does not extract from mid-sentence prose like 'No action items were discussed'."""
        summary = (
            "Within the last 12 hours, the group exchanged three messages. "
            "No action items were discussed in this conversation."
        )

        result = summarizer._extract_action_items(summary)
        assert result == []

    def test_extract_action_items_matches_section_header(self, summarizer):
        """Matches action items when formatted as a section header."""
...
- Create task in sprint backlog"""
        ...
        result = summarizer._extract_action_items(summary)
        assert len(result) == 2
        assert "Update the codebase" in result[0]

    def test_extract_action_items_matches_markdown_header(self, summarizer):
        """Matches action items with markdown formatting."""
...
class TestIsGenericActionItem:
    ...
    def test_detects_were_discussed(self, summarizer):
        """Detects 'were discussed' fragments as generic."""
        assert summarizer._is_generic_action_item("were discussed in this conversation.") is True
        assert summarizer._is_generic_action_item("were mentioned in the chat") is True

    def test_detects_there_were_no(self, summarizer):
        """Detects 'there were no' as generic."""
        assert summarizer._is_generic_action_item("There were no action items") is True
        assert summarizer._is_generic_action_item("There are no tasks") is True
...
class TestAnswerQuestion:
    ...
    def test_answer_question_empty_messages(self, summarizer):
        ...
        assert "No messages stored" in result

    def test_answer_question_success(self, summarizer, mock_ollama):
        """Answers question based on messages."""
        messages = [
            {"content": "Has anyone tried the new Python 3.12 features?"},
            {"content": "Yes! The f-string improvements are great"},
        ]
        mock_ollama.chat.return_value = "Python 3.12 features were discussed."

        result = summarizer.answer_question("What was discussed?", messages)

        assert "Python 3.12" in result
        mock_ollama.chat.assert_called_once()

    def test_answer_question_limits_context(self, summarizer, mock_ollama):
        """Uses limited context window from MAX_CONTEXT_WINDOW."""
        from summarizinator.ai.summarizer import MAX_CONTEXT_WINDOW
        messages = [{"content": f"Message {i}"} for i in range(100)]
        mock_ollama.chat.return_value = "Answer"

        summarizer.answer_question("Question?", messages)

        # Verify only the last MAX_CONTEXT_WINDOW messages are used
        call_args = mock_ollama.chat.call_args
        chat_messages = call_args.kwargs.get("messages") or call_args[1].get("messages")
        # The user prompt contains the conversation - check it doesn't have all 100
        user_prompt = chat_messages[1]["content"]
        assert f"Message {100 - MAX_CONTEXT_WINDOW}" in user_prompt
        assert "Message 0" not in user_prompt

    def test_answer_question_error(self, summarizer, mock_ollama):
        """Handles errors gracefully."""
        messages = [{"content": "Hello"}]
        mock_ollama.chat.side_effect = OllamaClientError("Error")

        result = summarizer.answer_question("Question?", messages)

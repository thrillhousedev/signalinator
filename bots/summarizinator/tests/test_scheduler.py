...
from summarizinator.scheduler.jobs import SummaryScheduler, _annotate_with_reactions
...
class TestAnnotateWithReactions:
    """Tests for the prompt-context reaction annotation helper."""

    def test_no_reactions_returns_content(self):
        assert _annotate_with_reactions({"content": "Hi", "emojis": []}) == "Hi"

    def test_missing_emojis_key_returns_content(self):
        assert _annotate_with_reactions({"content": "Hi"}) == "Hi"

    def test_with_reactions_appends_summary(self):
        out = _annotate_with_reactions({
            "content": "Hello",
            "emojis": ["👍", "❤️", "👍"],
        })
        assert out == "Hello [3 reactions: 👍❤️👍]"

    def test_empty_content_safe(self):
        assert _annotate_with_reactions({"content": "", "emojis": ["👍"]}) == " [1 reactions: 👍]"
...
class TestFormatSummaryMessage:
    ...
    def test_format_blank_line_between_topics_and_sentiment(self, mock_summary_scheduler):
        """A blank line separates Topics from Sentiment so they don't collapse together."""
        result = {
            "summary": "Summary text",
            "message_count": 10,
        ...
        topics_idx = output.index("📌 Topics:")
        sentiment_idx = output.index("💭 Sentiment:")
        between = output[topics_idx:sentiment_idx]
        # Topics line + at least one blank line before Sentiment.
    ...
    def test_format_includes_disclaimer(self, mock_summary_scheduler):
        """Includes AI accuracy disclaimer."""
        result = {
            "summary": "Summary text",
            "message_count": 10,
        }
        ...
        assert "verify for accuracy" in output.lower()
...
class TestGenerateSummaryNow:
    ...
    def test_on_demand_summary_honors_purge_on_summary(self, mock_summary_scheduler):
        """On-demand /summary must purge when purge_on_summary is on — same as scheduled."""
        mock_summary_scheduler.repo.get_messages_for_period.return_value = [
            MockMessage(id=1, content="Test")
        ]
        mock_summary_scheduler.repo.create_summary_run.return_value = MockSummaryRun(id=1)
        ...
        mock_summary_scheduler.generate_summary_now("test-group", hours=24)

        mock_summary_scheduler.repo.purge_messages.assert_called_with("test-group")

    def test_on_demand_summary_skips_purge_when_off(self, mock_summary_scheduler):
        """No purge when the setting is off."""
        mock_summary_scheduler.repo.get_messages_for_period.return_value = [
            MockMessage(id=1, content="Test")
        ]
        mock_summary_scheduler.repo.create_summary_run.return_value = MockSummaryRun(id=1)
        ...
        mock_summary_scheduler.repo.purge_messages.assert_not_called()

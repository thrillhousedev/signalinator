...
from ..bot import SnackinatorBot
...
@click.pass_context
def cli(ctx):
    """Snackinator - The Snack Oracle - wise, dry, judgment-free snack and meal suggestions via Ollama"""
...
@click.option('--phone', envvar='SIGNAL_PHONE_NUMBER', required=True)
@click.option('--config-dir', envvar='SIGNAL_CLI_CONFIG_DIR', default='/signal-cli-config')
@click.option('--name', default='snackinator')
...
@cli.command()
@click.option('--phone', envvar='SIGNAL_PHONE_NUMBER', required=True)
@click.option('--db-path', envvar='DB_PATH', default='/data/snackinator.db')
...
def daemon(phone, db_path, auto_accept_invites):
    """Run Snackinator daemon."""
    click.echo("Starting Snackinator daemon...")

    daemon_host = os.getenv("SIGNAL_DAEMON_HOST", "localhost")
    daemon_port = int(os.getenv("SIGNAL_DAEMON_PORT", "8080"))

    ollama_host = os.getenv("OLLAMA_HOST", "http://localhost:11434")
    ollama_model = os.getenv("SNACKINATOR_OLLAMA_MODEL") or os.getenv("OLLAMA_MODEL", "dolphin-mistral:7b")
    ...
    try:
        ...
        click.echo("\nSnackinator initialized — The Oracle is ready")
        click.echo("Commands: /help, /status")

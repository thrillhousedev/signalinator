...
Peer-bot output (messages and reactions) is filtered out of summaries by default. Peer bots are auto-discovered from sibling `<BOT>_PHONE` env vars exported by `env_file: .env`; their UUIDs are batch-resolved at startup via `SignalSSEClient.resolve_uuids_for_phones()` so sealed-sender messages (where `source_number` is `None`) are also caught.
...
### Daemon-side attachment cleanup

signal-cli writes received attachments to `/signal-cli-config/attachments/` inside each daemon container. Bots opt in to periodic cleanup by mounting that subdir writable into the bot container and setting:

- `SIGNAL_CLI_ATTACHMENTS_DIR` — path inside the bot container (typically `/signal-cli-attachments`)
- `SIGNAL_CLI_ATTACHMENTS_RETENTION_MINUTES` — keep files this long (default `5`; `0` means delete on next sweep)
- `SIGNAL_CLI_ATTACHMENTS_CLEANUP_INTERVAL` — sweep cadence in seconds (default `300`)

Eight bots ship with this enabled (`conductinator`, `decisionator`, `helpinator`, `newsinator`, `snackinator`, `summarizinator`, `taginator`, `transcribinator`). `informationator` deliberately omits the env vars and mount because it persists ingested attachments for RAG retrieval. The mount narrows to just the `attachments/` subdir of each daemon's config dir so private keys stay inaccessible to the bot container.

Lifecycle is automatic: `SignalCliAttachmentSweeper.from_env()` returns `None` when the env vars are unset, so unconfigured bots do nothing. When configured, the sweeper runs an initial sweep at startup (after `on_startup()`) and stops cleanly during shutdown.

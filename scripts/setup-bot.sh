...
# Available bots (parallel arrays for bash 3.2 compatibility on macOS)
BOTS=(        "conductinator"                        "decisionator"                          "informationator"                   "helpinator"              "helpinator2"              "newsinator"                            "snackinator"                              "summarizinator"              "taginator"                    "transcribinator")
BOT_DESCS=(   "Docker container management via Signal" "Loomio integration for polls/decisions" "RAG document Q&A (Ollama + ChromaDB)" "Signal help desk with ticketing"  "Signal help desk with ticketing (2)" "RSS/Reddit/Bluesky feed aggregation" "AI snack/meal recommendations (Ollama)" "AI-powered chat summaries" "@mention all group members"  "Audio/video transcription (Whisper)")
...
BOT_DISPLAY=$(echo "$BOT_NAME" | awk '{print toupper(substr($0,1,1)) substr($0,2)}')
...
echo "  4. Setting the profile photo (optional)"
echo "  5. Setting the Signal username"
echo "  6. Starting the bot"
echo ""
...
# Step 4: Avatar
print_header "Step 4: Profile Photo"

echo "You can set a profile photo (avatar) for the bot."
echo "Provide the path to an image file (PNG or JPG)."
echo ""

read -p "Path to avatar image (leave blank to skip): " AVATAR_PATH

if [ -n "$AVATAR_PATH" ]; then
    # Expand ~ if used
    AVATAR_PATH="${AVATAR_PATH/#\~/$HOME}"

    if [ ! -f "$AVATAR_PATH" ]; then
        print_warning "File not found: $AVATAR_PATH"
        echo "Skipping avatar. You can set it later with:"
        echo "  docker compose run --rm ${BOT_NAME}-daemon profile --name \"$DISPLAY_NAME\" --avatar /path/to/image.png"
        echo ""
        echo "Or via DM (while the bot is running):"
        echo "  /set-avatar (attach an image)"
    else
        print_step 4 "Setting avatar..."

        # Copy image into the daemon's config volume so it's accessible inside the container
        AVATAR_FILENAME="avatar$(echo "$AVATAR_PATH" | grep -o '\.[^.]*$')"
        cp "$AVATAR_PATH" "./config/${BOT_NAME}/${AVATAR_FILENAME}"

        docker compose run --rm "${BOT_NAME}-daemon" profile --name "$DISPLAY_NAME" --avatar "/signal-cli-config/${AVATAR_FILENAME}"
        echo -e "${GREEN}Avatar set!${NC}"
    fi
else
    echo "Skipping avatar. You can set it later with:"
    echo "  docker compose run --rm ${BOT_NAME}-daemon profile --name \"$DISPLAY_NAME\" --avatar /path/to/image.png"
...
# Step 6: Start
print_header "Step 6: Start"

if confirm "Start $BOT_NAME now?"; then
    print_step 6 "Starting $BOT_NAME..."

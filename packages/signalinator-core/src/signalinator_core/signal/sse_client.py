...
class SignalSSEClient:
    ...
    def send_message(
        ...
        """
        try:
            params = {"account": self.phone_number, "message": message or ""}
            if group_id:
    ...
    def resolve_uuids_for_phones(self, phones: List[str]) -> Dict[str, str]:
        """Resolve a batch of phone numbers to their Signal ACI UUIDs.

        Returns a {phone: uuid} map. Phones that can't be resolved are omitted.
        Used to identify peer bots whose messages arrive via sealed sender
        (source_number=None), where phone-based matching alone would miss them.
        """
        if not phones:
            return {}
        try:
            ...
            logger.warning(f"Failed to resolve peer UUIDs: {e}")
            return {}

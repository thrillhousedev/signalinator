...
from ..utils.attachments import SignalCliAttachmentSweeper
...
class SignalinatorBot(ABC):
    ...
    def __init__(
        ...
        # Daemon-side attachment sweeper (opt-in via env). Activated in run().
        self._signal_cli_sweeper: Optional[SignalCliAttachmentSweeper] = None
    ...
    def run(self) -> None:
        ...
        # Start daemon-side attachment sweeper if configured via env.
        self._signal_cli_sweeper = SignalCliAttachmentSweeper.from_env()
        if self._signal_cli_sweeper:
            self._signal_cli_sweeper.start()
    ...
    def _handle_group_message_internal(self, context: MessageContext) -> None:
        ...
        mentioned = self._router.is_bot_mentioned(context.mentions, self._bot_uuid, self.phone_number)
        mention_uuids = [m.get('uuid', '?')[:8] for m in context.mentions]
        logger.info(
            f"Group msg: mentions_count={len(context.mentions)}, "
            f"mention_uuids={mention_uuids}, "
            f"bot_uuid={self._bot_uuid[:8] if self._bot_uuid else 'None'}..., "
            f"bot_number={self.phone_number}, "
            f"mentioned={mentioned}"
        )

        # Fallback: Signal sometimes delivers @mentions with empty mentions array
        # (profile key exchange issue). Detect by \uFFFC placeholder + registered /command.
        if not mentioned and not context.mentions and context.message and '\uFFFC' in context.message:
            fallback_text = context.message.replace('\uFFFC', '').strip()
            command, _ = self._router.parse_command(fallback_text)
            if command and self._router.get_command(command):
                logger.info(f"Fallback mention: empty mentions array but found registered command {command}")
                mentioned = True

        if not mentioned:
            # Allow passive capture for bots that need it (e.g., Summarizinator)

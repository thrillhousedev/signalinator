...
class CommandRouter:
    ...
    def is_bot_mentioned(
        self, mentions: List[Dict], bot_uuid: str, bot_number: str = None
    ) -> bool:
        """Check if the bot is @mentioned in a message.

        Args:
            mentions: List of mention dicts from the message
            bot_uuid: The bot's UUID (ACI)
            bot_number: The bot's phone number (for fallback matching)

        Returns:
            True if the bot is mentioned
        """
        if not mentions:
            logger.info("is_bot_mentioned: mentions list is empty")
            return False
        for m in mentions:
            m_uuid = m.get('uuid')
            m_number = m.get('number')
            logger.info(
                f"is_bot_mentioned: comparing mention uuid={m_uuid[:8] if m_uuid else 'None'}... "
                f"number={m_number} against bot_uuid={bot_uuid[:8] if bot_uuid else 'None'}... "
                f"bot_number={bot_number}"
            )
            # Check UUID match (primary)
            if bot_uuid and m_uuid == bot_uuid:
                return True
            # Check phone number match (fallback for some signal-cli versions)
            if bot_number and m_number == bot_number:
                return True
        return False
    ...
    def extract_command_text(
        ...
        if not text:
            return ""
        if not mentions:
            # Strip orphaned mention placeholders when mentions array is missing
            return text.replace(self.MENTION_PLACEHOLDER, '').strip()

        # Filter to only bot mentions (by UUID or phone number), sort by position (reverse to preserve indices)
        bot_mentions = [
            m for m in mentions
            if (bot_uuid and m.get('uuid') == bot_uuid) or
               (bot_number and m.get('number') == bot_number)
        ]
    ...
    def route(
        ...
        try:
            ...
            if response:
                send_success = send_response(response)

                if not send_success:
                    logger.warning(f"Failed to deliver response for {context.command}")
                    if send_reaction:
                        send_reaction("⚠️")
                    return True

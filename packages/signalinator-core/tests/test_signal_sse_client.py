...
class TestSignalSSEClientMessaging:
    ...
    def test_send_message_to_group(self, client, mock_rpc_response):
        ...
        with patch.object(client, "_call_rpc", return_value=mock_rpc_response):
            ...
            assert result  # Returns timestamp (truthy)
    ...
    def test_send_message_no_target_returns_false(self, client):
        """Test that sending without target returns None on error."""
...
class TestSignalSSEClientProfile:
    ...
    def test_get_own_uuid_from_group(self, client):
        """Test getting own UUID via getUserStatus."""
        # Mock getUserStatus response
        user_status_response = [
            {"uuid": "my-uuid-123", "number": "+1234567890"}
        ]
        with patch.object(client, "_call_rpc", return_value=user_status_response):
            uuid = client.get_own_uuid()

            assert uuid == "my-uuid-123"
            client._call_rpc.assert_called_once_with(
                "getUserStatus",
                {"account": "+1234567890", "recipient": ["+1234567890"]}
            )
